...
namespace Assets.Encrypter
{
	...
	internal class DigitsArray
	{
		...
		internal void CopyTo(byte[] array, int offset, int length)
		{
			if (array == null)
			{
			...
			{
				m_data[j] = (DType)((array[i - 3] << 24) + (array[i - 2] << 16) + (array[i - 1] << 8) + array[i]);
				DataUsed++;
			}
	...
	public class BigInteger
	{
		...
		public static BigInteger GenerateRandom(int bytes)
		{
			return GenerateRandom(bytes, rng);
		}

		public static BigInteger GenerateRandom(int numberOfBytes, RandomNumberGenerator rng)
		{
			...
			DigitsArray digits = ret.m_digits;
			digits.Allocate(numberOfBytes / 4);

			digits.CopyTo(random, 0, numberOfBytes);
		...
		internal byte[] GetBytes()
		{
			...
			int numBits = m_digits.DataUsed * 8;
			...
			for (int i = (int)m_digits.Count - 1; i >= 0; i--)
			{
				uint val = m_digits[i];
